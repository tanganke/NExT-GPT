...
class TX2TInstructionDataset(Dataset):
    ...
    def __init__(
        self,
        data_path: str,
        mm_root_path: str = None,
        dataset_type: str = "ImageToText",
    ):
        ...
        for instance in tqdm(res, total=len(res)):
            ...
            if self.dataset_category == "tx2t":
                # Text + X -> Text dataset
                self.mm_path_list.append(
                    os.path.join(mm_root_path, instance["image_name"])
                )
        self.dataset_type_list = [
            dataset_type for _ in range(len(self.instruction_list))
        ]
    ...
    def __getitem__(self, i):
        if self.dataset_category == "tx2t":
            # Text + X -> Text dataset
            return dict(
                mm_paths=self.mm_path_list[i],
                output_texts=self.instruction_list[i],
    ...
    def collate(self, instances):
        if self.dataset_category == "tx2t":
            mm_paths, output_texts, dataset_types = tuple(
                ...
                for key in ("mm_paths", "output_texts", "dataset_types")
            )
            return dict(
                mm_paths=mm_paths,
            ...
            output_texts, dataset_types = tuple(
                [instance[key] for instance in instances]
                for key in ("output_texts", "dataset_types")
            )
            return dict(output_texts=output_texts, dataset_types=dataset_types)
