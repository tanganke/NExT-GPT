...
def process_batch_instance(
    ...
    for conversation in batch_of_conversations:
        if dataset == "pgpt4":
            _, one_input_ids, one_target_ids = build_one_instance_for_pgpt4(
                tokenizer, conversation
            )
        elif dataset == "cc3m" or dataset == "coco2017":
            _, one_input_ids, one_target_ids = build_one_instance_for_cc3m_1(
                tokenizer, conversation, num_img_tokens
            )
        elif dataset == "webvid":
            _, one_input_ids, one_target_ids = build_one_instance_for_webvid(
                tokenizer, conversation, num_video_tokens
            )
        else:
...
def mask_token(
    inputs, tokenizer, mlm_probability, vocab_size=None, special_tokens_mask=None
):
    """
...
def process_batch_stage_1(tokenizer, batch_of_captions, max_tgt_len, prompt=""):
    ...
    for caption in batch_of_captions:
        one_input_ids, one_target_ids = build_one_instance_stage_1(
...
def build_one_instance_stage_2(
...
def process_batch_stage_2(
    tokenizer, batch_of_captions, max_tgt_len, num_signal_tokens=4, MODALITY="image"
):
    """
    ...
    for captions in batch_of_captions:
        one_input_ids, one_target_ids = build_one_instance_stage_2(
            tokenizer, captions, num_signal_tokens=num_signal_tokens, MODALITY=MODALITY
        )
        batch_input_ids.append(torch.LongTensor(one_input_ids))
        batch_target_ids.append(torch.LongTensor(one_target_ids))
...
def build_one_instance_stage_3(
...
def process_batch_stage_3(
    ...
    # batch_caption_lists = []
    for conversation in batch_of_conversations:
        _, one_input_ids, one_target_ids, caption = build_one_instance_stage_3(
            tokenizer,
            conversation,
