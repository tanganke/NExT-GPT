...
class NextGPTModel(nn.Module):
    ...
    def __init__(self, **args):
        ...
        for layer_idx in self.args["text_emb_to_img_layers"]:
            if (
                layer_idx == -1
                or layer_idx == self.llama_model.config.num_hidden_layers
            ):
    ...
    def _add_image_token(self):
        ...
        for i in range(self.args["num_gen_img_tokens"]):
            ...
            print(
                f'Before adding new token, tokenizer("[IMG{i}]") =',
                self.llama_tokenizer(f"[IMG{i}]", add_special_tokens=False),
            )
            num_added_tokens = self.llama_tokenizer.add_tokens(f"[IMG{i}]")
            print(
                f'After adding {num_added_tokens} new tokens, tokenizer("[IMG{i}]") =',
                self.llama_tokenizer(f"[IMG{i}]", add_special_tokens=False),
            )
            gen_token_idx = self.llama_tokenizer(
                f"[IMG{i}]", add_special_tokens=False
            ).input_ids
    ...
    def _add_video_token(self):
        ...
        for i in range(self.args["num_gen_video_tokens"]):
            ...
            print(
                f'Before adding new token, tokenizer("[VID{i}]") =',
                self.llama_tokenizer(f"[VID{i}]", add_special_tokens=False),
            )
            num_added_tokens = self.llama_tokenizer.add_tokens(f"[VID{i}]")
            print(
                f'After adding {num_added_tokens} new tokens, tokenizer("[VID{i}]") =',
                self.llama_tokenizer(f"[VID{i}]", add_special_tokens=False),
            )
            gen_token_idx = self.llama_tokenizer(
                f"[VID{i}]", add_special_tokens=False
            ).input_ids
            assert len(gen_token_idx) == 1, gen_token_idx
    ...
    def _add_audio_token(self):
        ...
        for i in range(self.args["num_gen_audio_tokens"]):
            ...
            print(
                f'Before adding new token, tokenizer("[AUD{i}]") =',
                self.llama_tokenizer(f"[AUD{i}]", add_special_tokens=False),
            )
            num_added_tokens = self.llama_tokenizer.add_tokens(f"[AUD{i}]")
            print(
                f'After adding {num_added_tokens} new tokens, tokenizer("[AUD{i}]") =',
                self.llama_tokenizer(f"[AUD{i}]", add_special_tokens=False),
            )
            gen_token_idx = self.llama_tokenizer(
                f"[AUD{i}]", add_special_tokens=False
            ).input_ids
            assert len(gen_token_idx) == 1, gen_token_idx
    ...
    def encode_video(self, video_paths):
        ...
        inputs_llama = self.llama_proj(video_embeds).unsqueeze(
            1
    ...
    def encode_audio(self, audio_paths):
        ...
        inputs_llama = self.llama_proj(audio_embeds).unsqueeze(
            1
        )  # bsz x 1 x llama_size
        atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(
            self.device
    ...
    def prompt_wrap(self, img_embeds, input_ids, target_ids, attention_mask):
        ...
        if self.args["freeze_lm"]:
            p_after_embeds = self.llama_model.model.embed_tokens(input_ids).expand(
                batch_size, -1, -1
        ...
        if img_embeds is not None:
            ...
            empty_targets = (
                torch.ones(
                    [batch_size, 1 + p_before_embeds.size()[1] + 1],  # 1 (bos) + s1 + 1
                    dtype=torch.long,
                )
                .to(self.device)
                .fill_(-100)
            )  # bsz x (1 + s1)
            targets = torch.cat([empty_targets, target_ids], dim=1).to(
                self.device
            )  # bsz x (1 + s1 + 1 + s2)
            assert inputs_embeds.size()[1] == targets.size()[1]

            atts_prefix = torch.ones(
                [batch_size, 1 + p_before_embeds.size()[1] + 1], dtype=torch.long
            ).to(
                self.device
            )  # bsz x (1 + s1 + 1)
            attention_mask = torch.cat([atts_prefix, attention_mask], dim=1).to(
                self.device
            )
            ...
            inputs_embeds = torch.cat(
                ...
                self.device
            )  # bsz x (1+s1+s2) x embed_dim

            # create targets
            empty_targets = (
                torch.ones(
                    [batch_size, 1 + p_before_embeds.size()[1]],  # 1 (bos) + s1
                    dtype=torch.long,
                )
                .to(self.device)
                .fill_(-100)
            ...
            atts_prefix = torch.ones(
                [batch_size, 1 + p_before_embeds.size()[1]], dtype=torch.long
            ).to(
                self.device
            )  # bsz x (1 + s1)
            attention_mask = torch.cat([atts_prefix, attention_mask], dim=1).to(
    ...
    def _train_with_mode(
        ...
        if stage == 2:
            input_ids, target_ids, attention_mask = process_batch_stage_2(
                self.llama_tokenizer, texts, self.max_length, num_gen_tokens, modality
            )
        elif stage == 3:
        ...
        inputs_embeds, targets, attention_mask = self.prompt_wrap(
            img_embeds, input_ids, target_ids, attention_mask
        )
        ...
        if modality == "text":
            ...
            start_pos = (
                (targets == gen_token_idx[0]).nonzero(as_tuple=False)[:, 1].tolist()
            )
            end_pos = (
                (targets == gen_token_idx[-1]).nonzero(as_tuple=False)[:, 1].tolist()
            )
            # logging.info(f'targets : {targets}')
            # logging.info(f'start_pos : {start_pos}')
            # logging.info(f'end_pos : {end_pos}')
            assert (
                0 < len(start_pos) == len(end_pos) == input_ids.size(0)
                and len(end_pos) > 0
            ), (start_pos, end_pos)
            for idx, fc_layer in zip(text_emb_layers, text_hidden_fcs):
                ...
                hidden_states.append(
                    fc_layer(hidden_embedding, input_embedding)
                )  # (N, seq_len, 2048)
            ...
            if modality == "image":
                mse_loss = l2_loss(
                    embeddings,
                    torch.stack(text_prompt_embeddins, dim=0).to(self.device),
                )
            elif modality == "video":
                mse_loss = l2_loss(
                    embeddings,
                ...
                text_prompt_embeddins = torch.stack(text_prompt_embeddins, dim=0).to(
                    self.device
                )
                assert (
                    len(text_prompt_embeddins.shape) == 2
                ), text_prompt_embeddins.shape
                text_prompt_embeddins = text_prompt_embeddins.view(
                    text_prompt_embeddins.size(0), 1, text_prompt_embeddins.size(1)
                )
    ...
    def _enc_align_training_stage_1(self, inputs):
        ...
        if dataset_type == "ImageToText":
            ...
            raise NotImplementedError
        input_ids, target_ids, attention_mask = process_batch_stage_1(
            self.llama_tokenizer,
            inputs["output_texts"],
            self.max_length,
            self.args["prompt"],
        ...
        # print(input_ids)
        inputs_embeds, targets, attention_mask = self.prompt_wrap(
            mm_embeds, input_ids, target_ids, attention_mask
        )
        outputs = self.llama_model(
            inputs_embeds=inputs_embeds,
            attention_mask=attention_mask,
    ...
    def _dec_align_training_stage_2(self, inputs):
        ...
        if dataset_type == "TextToImage":
            loss, gen_acc, mse_loss = self._train_with_mode(
                texts=inputs["output_texts"],
                modality="image",
    ...
    def _instruction_tuning_stage_3(self, inputs):
        ...
        if dataset_type == "TextToImage":
            ...
            loss, gen_acc, _ = self._train_with_mode(
                inputs["output_texts"], img_embeds, modality="text", stage=self.stage
            )
        elif dataset_type == "TextToText":
            loss, gen_acc, _ = self._train_with_mode(
                inputs["output_texts"], None, modality="text", stage=self.stage
            )
        else:
            raise NotImplementedError
        return loss, gen_acc, mse_loss
    ...
    def forward(self, inputs):
        ...
        if self.stage == 1:
            ...
            raise NotImplementedError(
                f"stage {self.stage} is not implemented, now it only support [1, 2, 3]"
            )
    ...
    def _prepare_image_embed(self, text, batch_size):
        ...
        features = []
        p_before_token = self.llama_tokenizer(
            "<Img>", add_special_tokens=False, return_tensors="pt"
        ).to(self.device)
        p_after_token = self.llama_tokenizer(
            "</Img>", add_special_tokens=False, return_tensors="pt"
        ).to(self.device)
        if self.args["freeze_lm"]:
            p_before_embeds = self.llama_model.model.embed_tokens(
                p_before_token.input_ids
            ).expand(
                batch_size, -1, -1
            )  # bsz x s1 x embed_dim
            p_after_embeds = self.llama_model.model.embed_tokens(
                p_after_token.input_ids
            ).expand(
                batch_size, -1, -1
            )  # bsz x s2 x embed_dim
        else:
    ...
    def prepare_generation_embedding(self, inputs):
        ...
        for st in split_text:
            if st.startswith("Image>"):
                ...
                if self.args["freeze_lm"]:
                    text_embeds = self.llama_model.model.embed_tokens(
    ...
    def generate_tokens_embeddings(
        self, inputs, input_embeds, temperature: float = 0.0, top_p: float = 1.0
    ):
        """
        ...
        stopping_criteria = StoppingCriteriaList(
            [StoppingCriteriaSub(stops=inputs["stops_id"], encounters=1)]
        )
    ...
    def generate_images(
        ...
        last_ret_idx = 0
        return_outputs = []
        generation_model = StableDiffusionPipeline.from_pretrained(
            self.sd_ckpt_path, torch_dtype=torch.float16
        ).to("cuda")
        for gen_idx in all_gen_idx:
            assert (
                generated_ids[0, gen_idx : gen_idx + self.args["num_gen_img_tokens"]]
                .cpu()
                .detach()
                .numpy()
                .tolist()
            ...
            raw_emb = embeddings[
                :, gen_idx - 1 : gen_idx - 1 + self.args["num_gen_img_tokens"], :
            ]  # (1, 8, 4096)

            # Produce generation embedding.
            gen_prefix = " ".join(
                [f"[IMG{i}]" for i in range(self.args["num_gen_img_tokens"])]
            )
            gen_prefx_ids = self.llama_tokenizer(
                gen_prefix, add_special_tokens=False, return_tensors="pt"
            ).input_ids.to(self.device)
            gen_prefix_embs = self.input_embeddings(
                gen_prefx_ids
            )  # (1, T_I_V_A.txt, D)
            gen_emb = self.gen_text_hidden_fcs[-1](
                raw_emb, gen_prefix_embs
            )  # (1, 77, 768)
            ...
            if gen_emb.shape[1] != 77:
                ...
                gen_emb = torch.cat(
                    [
                        gen_emb,
                        torch.zeros(
                            (bs, 77 - seq_len, clip_emb),
                            device=gen_emb.device,
                            dtype=gen_emb.dtype,
                        ),
                    ],
                    dim=1,
            ...
            image_outputs = generation_model(
                prompt_embeds=gen_emb,
                guidance_scale=guidance_scale,
                num_inference_steps=num_inference_steps,
            ).images

            caption = self.llama_tokenizer.batch_decode(
                generated_ids[:, last_ret_idx:gen_idx], skip_special_tokens=True
            )[0]
    ...
    def generate_videos(
        ...
        return_outputs = []
        last_ret_idx = 0
        generation_model = TextToVideoSDPipeline.from_pretrained(
            self.vd_ckpt_path, torch_dtype=torch.float16
        ).to("cuda")
        for gen_idx in all_gen_idx:
            assert (
            ...
            raw_emb = embeddings[
                :, gen_idx - 1 : gen_idx - 1 + self.args["num_gen_video_tokens"], :
            ]  # (1, 8, 4096)
            ...
            # Produce generation embedding.
            gen_prefix = " ".join(
                [f"[VID{i}]" for i in range(self.args["num_gen_video_tokens"])]
            )
            gen_prefx_ids = self.llama_tokenizer(
                gen_prefix, add_special_tokens=False, return_tensors="pt"
            ).input_ids.to(self.device)
            ...
            gen_emb = self.gen_text_hidden_fcs_video[-1](
                raw_emb, gen_prefix_embs
            )  # (1, 77, 768)
            ...
            video_outputs = generation_model(
                prompt_embeds=gen_emb,
                guidance_scale=guidance_scale,
                num_inference_steps=num_inference_steps,
                height=height,
    ...
    def generate_audios(
        ...
        return_outputs = []
        last_ret_idx = 0
        generation_model = AudioLDMPipeline.from_pretrained(
            self.ad_ckpt_path, torch_dtype=torch.float16
        ).to("cuda")
        for gen_idx in all_gen_idx:
            assert (
            ...
            raw_emb = embeddings[
                :, gen_idx - 1 : gen_idx - 1 + self.args["num_gen_audio_tokens"], :
            ]  # (1, 8, 4096)
            # print(f'gen_idx: {gen_idx}')
            ...
            # Produce generation embedding.
            gen_prefix = " ".join(
                [f"[AUD{i}]" for i in range(self.args["num_gen_audio_tokens"])]
            )
            gen_prefx_ids = self.llama_tokenizer(
                gen_prefix, add_special_tokens=False, return_tensors="pt"
            ).input_ids.to(self.device)
            ...
            gen_emb = self.gen_text_hidden_fcs_audio[-1](
                raw_emb, gen_prefix_embs
            )  # (1, 77, 768)
            ...
            audio_outputs = generation_model(
                prompt_embeds=gen_emb,
                guidance_scale=guidance_scale,
                num_inference_steps=num_inference_steps,
    ...
    def generate(self, inputs):
        ...
        all_gen_img_idx = [
            ...
            for i, x in enumerate(
                generated_ids[0, :] == self.args["gen_img_token_idx"][0]
            )
            if x
        ][: inputs["max_num_imgs"]]
        ...
        all_gen_vid_idx = [
            i
            for i, x in enumerate(
                generated_ids[0, :] == self.args["gen_video_token_idx"][0]
            )
            if x
        ][: inputs["max_num_vids"]]
        ...
        all_gen_aud_idx = [
            i
            for i, x in enumerate(
                generated_ids[0, :] == self.args["gen_audio_token_idx"][0]
            )
            if x
        ][: inputs["max_num_auds"]]
        ...
        if (
            len(all_gen_img_idx) == 0
            and len(all_gen_vid_idx) == 0
            and len(all_gen_aud_idx) == 0
        ):
            ...
            if len(all_gen_vid_idx) > 0:
                vid_outputs = self.generate_videos(
                    generated_ids,
                    generated_video_embeddings,
                    all_gen_vid_idx,
                    None,
            ...
            if len(all_gen_aud_idx) > 0:
                aud_outputs = self.generate_audios(
                    generated_ids,
                    generated_audio_embeddings,
                    all_gen_aud_idx,
                    None,
