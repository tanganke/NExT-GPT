...
delta_ckpt = torch.load(
    os.path.join(args["nextgpt_ckpt_path"], f"pytorch_model.pt"),
        ...
        if isinstance(p, str):
            ...
            for m in p["img"]:
                if isinstance(m, str):
                    response += m.replace(
                        " ".join(
                            [f"[IMG{i}]" for i in range(args["num_gen_img_tokens"])]
                        ),
                        "",
                    )
                    response += "<br>"
                    _temp_output += m.replace(
                        " ".join(
                            [f"[IMG{i}]" for i in range(args["num_gen_img_tokens"])]
                        ),
                        "",
            ...
            for idx, m in enumerate(p["vid"]):
                if isinstance(m, str):
                    response += m.replace(
                        " ".join(
                            [f"[VID{i}]" for i in range(args["num_gen_video_tokens"])]
                        ),
                        "",
                    )
                    response += "<br>"
            ...
            for idx, m in enumerate(p["aud"]):
                if isinstance(m, str):
                    response += m.replace(
                        " ".join(
                            [f"[AUD{i}]" for i in range(args["num_gen_audio_tokens"])]
                        ),
                        "",
                    )
                    response += "<br>"
            ...
            pass
    response = (
        response[: -len("<br>")].rstrip() if response.endswith("<br>") else response
    )
...
def predict(
    ...
    user_chat, user_outputs = parse_text(
        prompt_input, image_path, video_path, audio_path
    )
...
with gr.Blocks() as demo:
            ...
            with gr.Tab("User Input"):
                with gr.Row(scale=3):
                    user_input = gr.Textbox(
                        label="Text", placeholder="Key in something here...", lines=3
                    )
                with gr.Row(scale=3):
                    with gr.Column(scale=1):
                        ...
                        )  # .style(height=200)  # <PIL.Image.Image image mode=RGB size=512x512 at 0x7F6E06738D90>
                    with gr.Column(scale=1):
                        audio_path = gr.Audio(type="filepath")  # .style(height=200)
                    with gr.Column(scale=1):
                        video_path = (
                            gr.Video()
                        )  # .style(height=200) # , value=None, interactive=True
        with gr.Column(scale=0.3, min_width=300):
            with gr.Group():
                ...
                with gr.Accordion("Image Advanced Options", open=True):
                    guidance_scale_for_img = gr.Slider(
                        1,
                        10,
                        value=7.5,
                        step=0.5,
                    ...
                    num_inference_steps_for_img = gr.Slider(
                        10,
                        50,
                        value=50,
                        step=1,
                ...
                with gr.Accordion("Video Advanced Options", open=False):
                    guidance_scale_for_vid = gr.Slider(
                        1,
                        10,
                        value=7.5,
                        step=0.5,
                    ...
                    num_inference_steps_for_vid = gr.Slider(
                        10,
                        50,
                        value=50,
                        step=1,
                ...
                with gr.Accordion("Audio Advanced Options", open=False):
                    guidance_scale_for_aud = gr.Slider(
                        1,
                        10,
                        value=7.5,
                        step=0.5,
                    ...
                    num_inference_steps_for_aud = gr.Slider(
                        10,
                        50,
                        value=50,
                        step=1,
                    ...
                    audio_length_in_s = gr.Slider(
                        ...
                        value=9,
                        step=1,
                        label="The audio length in seconds",
                        interactive=True,
                    )
    ...
    submitBtn.click(
        ...
        [chatbot, history, modality_cache, image_path, audio_path, video_path],
        show_progress=True,
    )
    ...
    emptyBtn.click(
        reset_state,
        outputs=[image_path, audio_path, video_path, chatbot, history, modality_cache],
        show_progress=True,
    )

demo.queue().launch(
    share=True, inbrowser=True, server_name="0.0.0.0", server_port=24004
)
